...
def preProcessFile(fileName, revise_format_file):
    ...
    with open(revise_format_file,'a') as file:
        ...
        fieldnames = ['unique_id','first_name','last_name','address_line','suburb_name','city','country','postcode','eaddress','domain','phone_number','origin']
            ...
            while i < k_len:  # i<10
                if i < 5:  # before city
                    ...
                    if not element[i].strip("\"").strip():
                        ...
                        singleData[keys[i].strip("\"")] = element[i].lower().strip("\"")
                    singleData[keys[i+1]] = "new zealand"
                    i += 1
                elif i == 6:  # postCode field
                    if not element[i].strip("\"").strip():
                        singleData[keys[i+1].strip("\"")] = "null"
                    else:
                        singleData[keys[i+1].strip("\"")] = element[i].lower().strip("\"")
                    i += 1
                elif i == k_len-3:  # i =7 email field
                    ...
                    if counter == 1:
                        ...
                        if not email:
                            ...
                            singleData[keys[i+1].strip("\"")] = email.lower().strip("\"")
                        if not domain:
                            singleData[keys[i+2].strip("\"")] = "null"
                        else:
                            singleData[keys[i+2].strip("\"")] = domain.lower().strip("\"")
                    else:
                        if not element[i].strip("\"").strip():
                            singleData[keys[i+1].strip("\"")] = "null"
                            singleData[keys[i+2].strip("\"")] = "null"
                            ...
                            singleData[keys[i+1].strip("\"")] = element[i].lower().strip("\"")
                            singleData[keys[i+2].strip("\"")] = element[i].lower().strip("\"")
                    i += 1
                    ...
                    if len(phone_number) > 0:
                        singleData[keys[i+2].strip("\"")] = tuple(i for i in phone_number)
                    else:
                        singleData[keys[i+2].strip("\"")] = "null"
                    break
