        ...
        with open(fileName, encoding = "ISO-8859-1") as f:
            ...
            for row in reader:
                ...
                for (k,v) in row.items():
                    ...
                    values = v.split("|")
                    # print(values)
                    output_keys_len = len(fieldnames) #output_keys_len = 13
                    singleData[fieldnames[0]] = values[0].strip("\"")
                    i = 1
                    ## to delete all the ",,," at the end of each row, len(keys)-1
                    while i < output_keys_len: #i<13  len()
                        tem_values = []
                        # fields before phones
                        if i < output_keys_len-5:  # if i < 8
                        ...
                        if i == output_keys_len-5:  # i = 8
                            em = []
                            values[i] = values[i].strip("\"")
                            if values[i]:
                                em.append(values[i])
                                singleData[fieldnames[i]] = tuple(s for s in em)
                            else:
                                singleData[fieldnames[i]] = "null"
                            # print(fieldnames[i],singleData[fieldnames[i]])
                            ...
                            if counter == 1:
                                ...
                                singleData[fieldnames[i+1]] = tuple(s for s in email_value)
                                singleData[fieldnames[i+2]] = tuple(s for s in domain_value)
                            else:
                                email_values = []
                                email_values.append(values[i])
                                singleData[fieldnames[i+1]] = tuple(s for s in email_values)
                                singleData[fieldnames[i+2]] = tuple(s for s in email_values)
                            i += 1
                        # when comes to phone field
                        elif i == output_keys_len-4:  # i == 9
                            j = i # j = 9
                            ...
                            if len(phone_number) > 0:
                                singleData[fieldnames[i+2]] = tuple(i for i in phone_number)
                            else:
                                singleData[fieldnames[i+2]] = "null"
                            break
                    origin_value = []
                    ...
                    for ele in singleData:
                        ...
                        if singleData[ele][0] == "null" or singleData[ele] == "null":
                            singleData[ele] = None
                        # print("singledata",singleData[ele])
