...
input is experian_neighbourly.csv
output is csv format of the original file: experian_neighbourly_csvFormat.csv
...
plus: transform the type of all fields from object to set
...
def preProcessFile(fileName, revise_format_file):
    ...
    with open(revise_format_file,'a') as file:
        ...
        fieldnames = ['unique_id','first_name','last_name','address_line','suburb','city','postcode','email','eaddress','domain','phone_number','origin']
...
def read_neighbourly_file(reader,writer,data,fieldnames,original_fieldnames_len):
    ...
    for row in reader:
        ...
        for element in records:
            singleData[keys[0]] = element[0].strip("\"")
            i = 1
            while i < k_len:  # i<10
                tem_values = []
                if i == k_len-3:  # i =7 email field
                    ...
                    if element[i]:
                        em.append(element[i])
                        singleData[keys[i]] = tuple(s for s in em)
                    else:
                    ...
                    if counter == 1:
                        ...
                        if not email:
                            ...
                            singleData[keys[i+1]] = tuple(s for s in email_value)
                        if not domain:
                            singleData[keys[i+2]] = "null"
                        else:
                            domain_value = []
                            domain_value.append(domain.lower().strip("\""))
                            singleData[keys[i+2]] = tuple(s for s in domain_value)
                    else:
                        if not element[i].strip("\"").strip():
                            ...
                            singleData[keys[i+2]] = tuple(s for s in tem_values)
                    i += 1
                    ...
                    if len(phone_number) > 0:
                        singleData[keys[i+2]] = tuple(i for i in phone_number)
                    else:
                        singleData[keys[i+2]] = "null"
                    break
...
if os.path.exists(revise_format_file):
    ...
    t2 = datetime.now()
    print("read data needs:", t2-t1)
else:
    data = preProcessFile(input_file, revise_format_file)
    t2 = datetime.now()
    print("extract data needs:", t2-t1)
