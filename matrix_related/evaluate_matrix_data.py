...
# calculate the specific matching score between TWO records
def calculate_two_recoeds_score(field_values, fields_len):
    ...
    for i in range(len(v0)):
        if not need_break:
            ...
            break
    field_score = score
    # print("field_score", field_score)


    return field_score

# compare the same group by comparing fields by fields
def compare_same_records(same_records):
    ...
    # compare from "first_name" to "phone_number"
    for i in range(3,len(fields)-1):
        ...
        for j in range(same_records_len):
            ...
            # original format like this: ('d',) OR ('d', 'f')
...
# extract same groups from csvFormat_output.csv file, and compare each pair each time,
# and get matching score from the pair
def extract_same_records(file):
    ...
    with open(file, encoding="ISO-8859-1") as f:
        ...
        for item in cluster_id:
            ...
            if len(same_group) == 2:
                total_score += 1
                # send the same pairs to compare_same_records method to calculate the
                 # specific matching score
        ...
        print("Number accurate_rate:", accurate_rate)
